...
public class SmbFile1Builder implements Builder<SmbFile1> {
	...
	protected static Logger LOG = LoggerFactory.getLogger(SmbFile1Builder.class);
	private SmbFile1Config clientConfig;
	
	public SmbFile1Builder(SmbFile1Config config) {
		this.clientConfig = config;
	}

	@SuppressWarnings("unchecked")
	public SmbFile1 build() {
		
		//基于smb协议的共享文件访问对象
		SmbFile1 smbClient = null;
		...
		try {
			...
			if(StringUtils.isNotEmpty(clientConfig.getDomain())){
				...
				//创建基于smb协议的共享文件访问对象
				smbClient = new SmbFile1( SMBPathUtils.getSharedURL(clientConfig.getHost(), clientConfig.getSharedDir()) , authentication);
			}else{
				//共享目录访问路径
				...
				//创建基于smb协议的共享文件访问对象
				smbClient = new SmbFile1(sharedURL);
			}
	...
	public SmbFile1Config getConfiguration() {
		return clientConfig;
	}
