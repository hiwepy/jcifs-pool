...
public class SmbFile2StreamUtils {
    ...
    protected static Logger LOG = LoggerFactory.getLogger(SmbFile2StreamUtils.class);
	
    public static long copyLarge(SmbFile2 sharedFile,OutputStream output) throws IOException {
    	return SmbFile2StreamUtils.copyLarge(sharedFile, output, 0);
    }
     ...
     * @throws IOException
     */
    public static long copyLarge(SmbFile2 sharedFile,OutputStream output,long skipOffset) throws IOException {
		...
		try {
			...
			LOG.info("连接成功."); 
			//获得共享文件的输入流，以便进行读取
			input = SmbFile2StreamUtils.toBufferedInputStream(sharedFile.getInputStream(), sharedFile.getBufferSize());
			try {
				// 跳过已经存在的长度,实现断点续传  
				SmbFile2StreamUtils.skip(input, skipOffset);
			} catch (Exception e) {
    ...
    public static long copyLarge(InputStream input, SmbFile2 sharedFile) throws IOException {
    	return SmbFile2StreamUtils.copyLarge(input, sharedFile, 0);
    }
    
    
    public static void makeSharedDir(SmbFile2 sharedDir) throws IOException{
    	//目录不存在则继续
    	if(!sharedDir.exists()){
    		//共享的根目录
    		...
    		for (String tmpDir : dirList) {
    			...
    			//父级目录
        		SmbFile2 parentDir = sharedDir.wrap(new SmbFile(rootPath + tmpDir));
        		if(!parentDir.exists()){
        			parentDir.mkdir();
        		}else{
        			continue;
     ...
     * 拷贝本地文件到共享文件服务器
     */
	public static long copyLarge(InputStream input,SmbFile2 sharedFile,long skipOffset) throws IOException {
		...
		try {
			...
			if(!sharedFile.exists()){
				SmbFile2 sharedDir = new SmbFile2(SMBPathUtils.getResolvePath(sharedFile.getParent()));
